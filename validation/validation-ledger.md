...
| 2026-05-26 | Codex / GPT-5 | Local deployment inspection and Hermes context/tool-envelope correction for `chapters/09-local-agent-arc-b70.md` | partially verified; local findings recorded | SRC-09-003 through SRC-09-005 plus local deployment evidence | Restored imported source wording, appended a dated correction: current Hermes enforces a 64,000-token minimum and uses 65,536; smaller active tool/history payloads improved live tool-call responsiveness in a single local probe pass. Also identified non-contained local terminal execution and paused one mutating cron task. | Run repeated benchmark-plan probes, capture internal benchmark artifacts, validate sandboxed execution, and resolve Intel GuC firmware warning before performance promotion. |
