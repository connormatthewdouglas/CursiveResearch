...
| Chapter 16 / network headline | The +450–900% network delta demonstrates transport-stack behavior (chiefly CUBIC→BBR under emulated 50ms/0.5%-loss loopback) and does not yet establish real-path WAN/mining/inference gains. | Supported | 2026-06-11 | Harness code labels the comparison CUBIC vs BBR on loopback netem; BBR-vs-CUBIC under random loss is documented transport behavior. No real-path A/B exists. | Keep the WAN-simulation qualifier on all public claims; run the real-path network A/B experiment before any broader claim. |
