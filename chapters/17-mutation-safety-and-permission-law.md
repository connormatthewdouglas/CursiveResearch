...
Chapter 13 (Gap 2, "What Should Be Added Next" #1) flagged that the corpus had
the *pieces* of a mutation-safety model — reversible presets, regression gates,
the daemon/shell split (Chapter 12) — but no consolidated, source-backed
**permission law**. Chapter 06 is the closest neighbor, but it solves the
...
Chapter 13 defined a seven-class mutation taxonomy. The new contribution here is
...
Chapter 12 established the daemon/shell trust boundary; the permission law makes
...
- **Firmware staging** — class-6 changes use the platform's pending-settings +
  reset-required model (UEFI/Redfish, see Chapter 08) so firmware mutation is
