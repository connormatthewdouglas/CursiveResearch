...
- Real-path network decomposition: BBR swap dominates; buffer stack ~0% on â‰¤1GbE loss

### Superseded or narrowed
- Loopback +246% stack-delta as transferable WAN claim â€” **Disproven**; mechanism-only (Ch09)
- Idle-power CV 0.83 as inherent noise â€” **Disproven**; sampling artifact (settled idle CV â‰ˆ 0.01)

### Open until experiment/hardware
- Cold-start order/cache counterbalance (`experiments/cold-start-order-cache-confound-plan.md`)
- `power_source` and `page_cache_state` in structured output (Â§3 items 2, 4)

---

## Reinforced research (2026-06-24)

- **Agent benchmark discipline:** Kapoor et al., *AI Agents That Matter* (2024) â€” intake `papers/agent-evaluation/ai-agents-that-matter/`; cost, holdouts, and simple baselines must gate any Ch05 shell benchmark design.
- **Proxy / Goodhart risk:** Skalse et al., "Defining and Characterizing Reward Hacking" (NeurIPS 2022) â€” `papers/recursive-self-improvement/reward-hacking-skalse-2022/`; single-channel fitness (network emulation) is high Goodhart surface.
- **Sequential decision-making:** Spotify Engineering (2023), [Choosing a Sequential Testing Framework](https://engineering.atspotify.com/2023/03/choosing-sequential-testing-framework-comparisons-and-discussions) â€” supports pre-registered counterbalance before acceptance-grade cold-start claims (Ch08).
- **Transport measurement scope:** Linux `Documentation/networking/tcp-bbr.rst` and Cardwell et al. BBR (ACM Queue 2017) â€” algorithm-vs-buffer decomposition in Â§5 item 6; ESnet/iperf3 guidance for labeled real-path vs loopback tests.
...
The organism's entire truth model (Chapter 01) reduces to: *sensor result +
schema validity + confidence + gates + population confirmation = evidence*.
That chain is only as strong as the measurement at its base.

> **Corpus inline (2026-06-24):** Loopback +246% stack-delta is **not** transferable WAN evidence (Ch09); dominant ≤1GbE win is CUBIC→BBR (Cardwell et al. 2017). Cold-start magnitudes are hardware-scoped (Ch00 §5 item 5; Ch08).
...
| --- | --- | --- | --- |
| Network throughput | iperf3 over **loopback** with `tc netem` (50ms RTT, 0.5% loss); compares **CUBIC baseline vs BBR-tuned**; 5 passes | baseline/tuned/delta (single values) | per-pass rates, retransmits, RTT in logs â†’ `run_detail_bundles` when the seed path runs |
| Cold-start latency | GPU idle â†’ first token; load duration, TTFT | baseline/tuned/delta | per-call GPU-before frequency, load, TTFT in logs |
...
| Idle power | median of up to 5 Ã— 1-second readings per condition | baseline/tuned/delta watts | raw sample arrays in result JSON |
| Stability | dmesg errors, throttle events, stability flag | counts + flag in notes/columns | summary log |
| Identity | hardware fingerprint v2 (CPU model, board, GPU PCI ids) | `machine_id` joins `machines`/`machine_aliases` | â€” |
...
### 2.1 Network (+450â€“900% headline) â€” real effect, narrow scope
...
stack (chiefly BBR) sustains 5â€“9Ã— the throughput of default CUBIC." Claims
...
### 2.2 Idle power â€” the least comparable channel

`read_watts` selects, in priority order: Intel RAPL package energy â†’ AMD
powercap â†’ GPU hwmon energy counter â†’ hwmon instantaneous power â†’
...
- **Different machines report different physical quantities** (CPU package
  ...
  structured result** â€” it only appears in stderr guard logs. Cross-machine
...
### 2.3 Cold-start and sustained inference â€” honest but under-described
...
- Order is fixed (baseline first, tuned second) within a session; thermal
  ...
  counterbalanced repeats before acceptance â€” correct, keep it.
...
- `sample_counts` in the result JSON hardcodes `network: 1` although the
  network benchmark performs 5 internal passes â€” under-reporting evidence.
...
- `notes` is a parseable-by-regex string carrying load-bearing data
  (`hw:`, `stability:`, `kernel:`) â€” workable, but fragile as schema.
...
2. **Promote per-pass arrays into `run_detail_bundles` on every run**, not
   ...
   within-session variance â†’ honest confidence instead of `0.5` constants.
...
Items 1â€“4 and 7 are main-repo build tasks (graduate via the normal rule);
items 5â€“6 are experiments (`experiments/` plans exist or should be added).
...
(Chapter 01/15) in the one place it is currently concrete.
...
**What this adds to the chapter**: Recursive self-improvement and organism measurement rely on proxy optimization (benchmark deltas, fitness for selection), and Â§4 above flags exactly where that proxy is currently gameable. Skalse et al. supplies the rigorous theoretical backbone for that empirical concern â€” formal conditions for when proxy optimization is and is not safe â€” which future fitness/schema work can reference when designing combined or robust proxies. The paper fills that foundational gap without overclaiming transfer to noisy real hardware.
...
1. **Attribution by complementary ablation works.** v0.9b (GPU frequency pin
   only â€” pin *verified active at 2000 MHz* via the new phase-context
   telemetry) produced zero cold-start improvement; v0.9c (full v0.8 minus
   the GPU pin) retained the full âˆ’51% cold-start win with equivalent
...
2. **The power-source warning (Â§2.2) was confirmed in production.** The rig
...
3. **Metric split implemented** (main repo): the legacy CUBIC-vs-BBR netem
   ...
   CursiveOS buffer/qdisc tuning â€” the number attributable to the project's
   own work. netem application is now verified before measurement (item 7
   of Â§3). Public iperf3 endpoints proved unreliable for real-path testing;
   a LAN endpoint remains the practical path.

4. **Stack-delta result corrects the Â§2.1 assumption (2026-06-13).** Running
   ...
   Decomposing the legacy ~+800% total (untuned CUBIC ~150 â†’ fully tuned
   ~1367 Mbit/s â‰ˆ 9Ã—): the CUBICâ†’BBR swap alone is ~2.6Ã— (+160%), and the
   buffer/qdisc tuning on top of BBR is ~3.5Ã— (+246%); 2.6 Ã— 3.5 â‰ˆ 9Ã—. So
   Â§2.1's original framing ("much of the +500% is attributable to one sysctl,
   bbr") was **wrong on the magnitude split** â€” on a high-BDP path the buffer
   ...
   **Caveat that still holds:** this is loopback. netem manufactures a
   ...
   window below that BDP â€” a real and well-understood mechanism. But the
...
5. **Cold-start optimization is hardware-scoped (2026-06-13, second machine).**
   ...
   | --- | --- | --- |
   | Ryzen 7 5700 + Arc A750 (desktop) | âˆ’51% | âˆ’51% |
   | i5-11300H (laptop) | ~0% (âˆ’0.1, âˆ’6.3) | +0.4% |

   Crucially, the phase-context telemetry **rules out the obvious confounds**:
   on the laptop the governor did change `powersave â†’ performance`, the
   ...
   that gives the Arc desktop âˆ’51%. (The laptop's own May run showed âˆ’29%,
   so its small benefit is also unstable across conditions.) This is the
   project's **first empirical instance of hardware-scoped fitness** â€” the
   exact scenario Chapter 01's evidence model anticipates: a variant that
   helps one hardware class must not be promoted as a global gain. It also
   demonstrates the payoff of the Â§3 telemetry additions: without governor
   and AC state in the record, "laptop doesn't benefit" would be
   indistinguishable from "tweak didn't stick / ran on battery."

   Implication for v0.9c: on **both** machines v0.9c â‰¡ v0.8 (laptop: both
   ~0 cold; desktop: both âˆ’51%), so dropping the GPU pin costs nothing
   anywhere â€” v0.9c remains a safe global parent replacement. But the
...
6. **Real-path A/B overturns the stack-delta magnitude (2026-06-16).** The
   ...
   loss present (the key â€” earlier delay-only runs showed nothing because no
   buffer pressure exists without loss):
   - CUBIC + host-default buffers: **43.1 Mbit/s** (collapses under loss)
   - BBR + host-default buffers: **851.1 Mbit/s** â€” algorithm gain **+1875%**
   - BBR + CursiveOS buffer stack: **845.0 Mbit/s** â€” our-stack gain **âˆ’0.7%**

   On a real â‰¤1GbE link the **entire** real-world network win is the CUBICâ†’BBR
   ...
   **loopback artifact**. Loopback's "link" is memory bandwidth (tens of Gbit/s),
   ...
   real default buffers cannot fill â€” and our 16 MB buffers helped. A real
   1GbE link at 50 ms has BDP â‰ˆ 6 MB, which default Linux autotuning already
   ...
   is dead weight on ordinary â‰¤1GbE hardware and earns its keep only on
...
7. **Per-channel noise floor measured â€” the first fleet-variance data (2026-06-16).**
   Six identical v0.9 full-tests on one machine (Stardust) give the within-machine
   noise floor that the population-confirmation rule (Chapter 01) needs:

   | Channel | Mean | Std | CV | Note |
   | --- | --- | --- | --- | --- |
   | Cold-start | âˆ’50.8% | 0.1 | **0.002** | rock-solid; the project's most reliable signal |
   | Network | 707% | 136 | **0.192** | above the 0.15 CV threshold; range 602â€“970% |
   | Sustained | âˆ’0.45% | 0.51 | sign-unstable | signal < noise; unusable single-stream |
   | Idle power (CPU) | 4.02 W | 3.32 | **0.83** | near-random as currently measured |

   Implications, concrete: (a) **cold-start needs ~1 confirmation** (CVâ‰ˆ0); the
   âˆ’51% Arc win is extremely repeatable. (b) **Network genuinely triggers the
   CV>0.15 escalation** (N+2 confirmations) and its *magnitude* should never be
   quoted precisely â€” only its presence. (c) **Sustained single-stream and idle
   power cannot gate selection as measured** â€” sustained is below its own noise,
   ...
   A750 via `hwmon*/energy1_input` (~37 W idle), closing the Â§2.2 blindspot on
   this hardware: total power (CPU package + dGPU â‰ˆ 55 W) is now measurable, so
   the true cost of a GPU pin can finally be quantified against v0.8 in a
   future run.

8. **GPU-pin power cost â‰ˆ 0 W, and the power-noise was a sampling artifact (2026-06-16, Phase D).**
   A dedicated total-power probe (CPU RAPL + GPU energy, 12 settled samples per
   state, Ã—2 reps) measured the v0.8 Arc GPU pin (slpc_ignore_eff + rps_min
   2000 + rps_boost max) vs unpinned (v0.9):
   - Unpinned (v0.9): CPU 5.49 W + GPU 36.66 W = **42.15 W**
   - Pinned (v0.8): CPU 5.49 W + GPU 36.67 W = **42.16 W**  â†’ **pin cost â‰ˆ 0 W**

   Two conclusions: (a) the GPU pin is inert at idle â€” it costs ~0 W, so v0.9's
   ...
   power remains untested. (b) **The CV 0.83 idle-power noise from Â§5 item 7 is
   ...
   runs; CV â‰ˆ 0.01). The full-test's noisy power came from sampling during the
   ...
   channel after all â€” once measured correctly.

9. **Cold-start order/page-cache confound â€” open, highest-leverage validity gap (2026-06-23).**
...
- Cold-start is repeatable (CV 0.002) but **not yet order-robust** (Â§5 item 9).
...
- The v0.9 screen verdict logic (analyzer) is sound, but its power term
  inherits Â§2.2's comparability limits â€” fine for same-machine screens,
  not for cross-machine pooling.
- Population confirmation (N rule, CV â‰¤ 0.15) should be calibrated only on
