...
| Concurrency inference sensor noise floor | Chapter 01 / 10 | Prototype `benchmark-inference-concurrency-v0.1.sh` — aggregate tok/s under N parallel Ollama streams. | Kickoff (`experiments/concurrency-inference-sensor-noise-floor-plan.md`); observe-only in harness v1.4.5 (weight 0). | Validate CV on Stardust + laptop before any fitness weight. |
