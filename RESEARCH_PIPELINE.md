...
| Seed organism parent-vs-candidate screen | Chapter 01 / main repo | Run parent-vs-candidate screens via the one-paste seed session. | Cycle 3 closed: v0.11 accepted; parent v0.12. v0.12b **rejected** 2026-06-28. | Next candidate axis TBD (governor, load-power knob). |
...
| Idle-power selection-channel validation | Chapter 22 / 10 | Test whether the 2026-06-16 harness idle-power fix (settle + more samples) makes idle power a selection-usable channel (CV ≤ 0.15) in the production full-test path across hardware. | **Partial (2026-06-28):** Stardust H1 **PASS** (CV 0.016); laptop AC **FAIL** (cold run-1); H3 **PASS** (no cross-machine pooling). Weight **0**. | Laptop battery cohort + drop-first-run rule; do not pool idle power across machines. |
...
| Concurrent inference throughput benchmark | Chapter 22 / action plan parking lot | Measure requests/sec under parallel load, where scheduler tweaks should actually show. | Shipped; H1/H2 pass; H3 blocked; **weight 0**. v0.13-sched **rejected**. | Load-time power or swappiness axis; `tools/rig-smoke.sh` for rig automation. |
| Memory-pressure sensor noise floor | Chapter 01 / 00 / 08 | Validate the `benchmark-memory-pressure-v0.2.sh` refault-time probe across machines and gate it into fitness as a fifth channel only if CV ≤ 0.15. | Done / validated (`experiments/memory-pressure-sensor-noise-floor-plan.md`). | Integrated in harness v1.4.5; v0.11 accepted cycle 3; parent v0.12. |
| Concurrency inference sensor noise floor | Chapter 01 / 10 | Prototype `benchmark-inference-concurrency-v0.1.sh` — aggregate tok/s under N parallel Ollama streams. | H1/H2 passed 2026-06-27; H3 blocked (0% v0.8 vs v0.12); weight 0. | Scheduler-axis candidate screen; load-time power if scheduler null. |
...
| Sabotage-tolerance of confirmation (adversarial) | Chapter 08 / 03d / 03c | Test whether the Ch08 N-rule + CV escalation + effective-N downgrade actually resists saboteurs (false promotion of an inert variant) and griefers (false suppression of a real win) under measured per-channel noise — the 03c "decentralized evaluator" frontier. | Proposed (`experiments/sabotage-tolerance-confirmation-simulation-plan.md`). | Monte-Carlo the Ch08 rule seeded by Ch00 CVs (cold-start 0.002, network 0.192) and the hardware-scoped −51% cold-start effect; sweep saboteur fraction f and fleet_size across four attack models (independent liars, Sybil cluster, Goodhart gamer, griefer); report false-promotion/rejection curves with/without immune downgrade before any Ch02 BTC-gating. |
